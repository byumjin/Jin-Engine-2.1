...
public:
	...
	void updateLocalBuffer(PlaneInfoPack &planeInfoPack)
	{
	...
	void mapCullingInfo(std::vector<PlaneInfo> &planeArray)
	{
		VkDeviceSize bufferSize = sizeof(PlaneInfo) * planeArray.size();

		void* data;
		vkMapMemory(vulkanApp->getDevice(), planeInfoBufferMem, 0, bufferSize, 0, &data);
		memcpy(&planeArray[0], data, static_cast<size_t>(bufferSize));
		vkUnmapMemory(vulkanApp->getDevice(), planeInfoBufferMem);
	}
