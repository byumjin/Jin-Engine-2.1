...
void Renderer::initialize(Vulkan* pVulkanApp)
{
	...
	*/
	...
	//Depth Mipmap
	{
		...
		/*
		DepthMipCompMaterial* depth_mipmap_comp_Mat = new DepthMipCompMaterial;

		PostProcess *DepthMipComposition_PP = new PostProcess(vulkanApp, "DMC_mat", depthFormat, 1, singleTriangularVertexBuffer, VK_FILTER_NEAREST, VK_SAMPLER_MIPMAP_MODE_NEAREST);

		glm::vec4 sizeScale = glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);

		DepthMipComposition_PP->initialize(sizeScale);

		depth_mipmap_comp_Mat->createPipeline("DMC_mat", "", "", "", "", NULL, NULL,
			NULL, 0, NULL, 0, glm::vec2(0.0), DepthMipComposition_PP->sizeScale, NULL, NULL, NULL);

		DepthMipComposition_PP->recordCommandBuffer();

		postProcessChain.push_back(DepthMipComposition_PP);
		*/
	...
	//PBR material
	{
		...
		PostProcess *PBR_PP = new PostProcess(vulkanApp, "uber_mat", VK_FORMAT_R16G16B16A16_SFLOAT, 1, singleTriangularVertexBuffer, VK_FILTER_LINEAR, VK_SAMPLER_MIPMAP_MODE_LINEAR, false);
	...
	//SSR material
	{
		...
		PostProcess *SSRP_PP = new PostProcess(vulkanApp, "ssrp_mat", VK_FORMAT_R32G32B32A32_SFLOAT, 1, singleTriangularVertexBuffer, VK_FILTER_NEAREST, VK_SAMPLER_MIPMAP_MODE_LINEAR, true);

		SSRP_PP->initialize(sizeScale);

		std::vector<Texture*> tempSSRPRenderTargets;
		tempSSRPRenderTargets.push_back(postProcessChain[postProcessChain.size() - 1]->renderTargets[0]);
		tempSSRPRenderTargets.push_back(SSRP_PP->renderTargets[0]);
		tempSSRPRenderTargets.push_back(gbuffers[NORMAL_COLOR]);
		...
		SSRP_Mat->createPipeline("ssrp_mat", "", "", "", "", NULL, &mainCamera.uniformCameraBuffer, NULL, pointLightInfo.size(), NULL, directionalLightInfo.size(), NULL,
			glm::vec2(0.0), glm::vec4(swapChainExtent.width, swapChainExtent.height, 1.0, 1.0),
		...
		planeInfoPack.planeInfo[0].size = glm::vec4(30.0, 30.0, 0.0, 0.0);

		glm::mat4 basicMat;
		basicMat[0] = glm::vec4(1.0, 0.0, 0.0, 0.0); //tan
		basicMat[1] = glm::vec4(0.0, 0.0, -1.0, 0.0); //bitan
		basicMat[2] = glm::vec4(0.0, 1.0, 0.0, 0.0); //normal
		basicMat[3] = glm::vec4(0.0, 0.0, 0.0, 1.0);

		planeInfoPack.planeInfo[0].rotMat = glm::rotate(basicMat, glm::radians(0.0f), glm::vec3(1.0, 0.0, 0.0));

		SSRP_Mat->updateLocalBuffer(planeInfoPack);
		...
		std::string noiseTex = "Asset/Texture/noise/tex_BlueNoise_1024x1024_UNI.tga";
		
		ScreenSpaceReflectionMaterial* temp_ssr_Mat = new ScreenSpaceReflectionMaterial;

		PostProcess *SSR_PP = new PostProcess(vulkanApp, "ssr_mat", VK_FORMAT_R16G16B16A16_SFLOAT, 1, singleTriangularVertexBuffer, VK_FILTER_LINEAR, VK_SAMPLER_MIPMAP_MODE_LINEAR, false);
		...
		tempRenderTargets.push_back(postProcessChain[postProcessChain.size() - 2]->renderTargets[0]);
		tempRenderTargets.push_back(gbuffers[SPECULAR_COLOR]);
		tempRenderTargets.push_back(gbuffers[NORMAL_COLOR]);
		...
		temp_ssr_Mat->createPipeline("ssr_mat", "", "", "", "", NULL, &mainCamera.uniformCameraBuffer,
			&pointLightUniformBuffer, pointLightInfo.size(), &directionalLightUniformBuffer, directionalLightInfo.size(), NULL,
			glm::vec2(0.0), SSR_PP->sizeScale, SSR_PP->getRenderPass(), &tempRenderTargets, NULL);
...
void Renderer::createDepthResources()
{
	...
	for(size_t i = 0; i < depthMipTexture.size(); i++)
	{
		VkFormat depthMipFormat = VK_FORMAT_R32_SFLOAT;

		vulkanApp->createImage(VK_IMAGE_TYPE_2D, swapChainExtent.width / (uint32_t)pow(2, i + 1), swapChainExtent.height / (uint32_t)pow(2, i + 1), 1, 1, 1, depthMipFormat, VK_IMAGE_TILING_OPTIMAL,
			VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_SAMPLE_COUNT_1_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthMipTexture[i]->textureImage, depthMipTexture[i]->textureImageMemory);
		vulkanApp->createImageView(depthMipTexture[i]->textureImage, VK_IMAGE_VIEW_TYPE_2D, depthMipFormat, VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1, depthMipTexture[i]->textureImageView);
		vulkanApp->transitionImageLayout(depthMipTexture[i]->textureImage, depthMipFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, gbufferCmdPool, vulkanApp->getTransferQueue());
		vulkanApp->createTextureSampler(VK_FILTER_NEAREST, VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, VK_FALSE, 1, VK_BORDER_COLOR_INT_OPAQUE_BLACK, VK_FALSE,
			VK_SAMPLER_MIPMAP_MODE_NEAREST, 0.0f, 0.0f, 0.0f, depthMipTexture[i]->textureSampler);
	}
	*/
...
void Renderer::releaseDepthResources()
{
	...
	for (size_t i = 0; i < depthMipTexture.size(); i++)
	{
		vkDestroySampler(vulkanApp->getDevice(), depthMipTexture[i]->textureSampler, nullptr);
		vkDestroyImage(vulkanApp->getDevice(), depthMipTexture[i]->textureImage, nullptr);
		vkDestroyImageView(vulkanApp->getDevice(), depthMipTexture[i]->textureImageView, nullptr);
		vkFreeMemory(vulkanApp->getDevice(), depthMipTexture[i]->textureImageMemory, nullptr);
	}
	*/
